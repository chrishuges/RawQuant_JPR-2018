---
title: "Analysis of Q-Exactive from Bekker-Jensen *et al.*"
...
This file contains the code for processing the search and quantification output files from the HeLa in-depth proteome publication from Bekker-Jensen *et al.* (PMID: 28601559). **These data were used to make portions of Figure 1 and 2.**
...
First we can look at the distribution of MS2 events to see whether the distribution of events is even across the fractions. For this, we use the parsed files from RawQuant from MS level 2.  

```{r}
#first grab the files that are output from RawQuant for MS level 2
ms2_quant_files = list.files('/projects/ptx_results/OtherDataSets/2017-June_Jensen_CellSystems/hela_1mg/quant-output',pattern='*MS2ParseData.txt',full.names=TRUE)

#now look at the distribution of MS2 events
ms2_data = data.frame() #data storage object 
#iterate over the quantification files to get the numbers of MS2 scans from all files into a single object
for (i in 1:length(ms2_quant_files)){
  ms2_in = read.table(ms2_quant_files[i],header=TRUE,sep='\t') #read in the input file
  ms2_in$fraction = sub(".*?Rep1_(.*?)_MS2ParseData.*", "\\1", ms2_quant_files[i]) #add a fraction column based on the file name
...
saveRDS(ms2_data,'/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-jensen_all-fractions-ms2-data.rds') #save the data
...
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-number-of-msms-46fractions.pdf',msms_plot) #save the plot
message('mean number of msms = ',mean(msms_table$Freq,na.rm=TRUE))
```

Hmm...the distribution doesn't look very even. How does this translate to identifications? 
...
psm_files = list.files('/projects/ptx_results/OtherDataSets/2017-June_Jensen_CellSystems/hela_1mg/reports-output',pattern='*zip_1_Default_PSM_Report.txt',full.names=TRUE) #locate the desired files
...
for (i in 1:length(psm_files)){
  ...
  psm_f1 = psm_in %>% 
    ...
    dplyr::mutate(Spectrum.Title, fraction = sub(".*?Rep1_(.*?)_MGF\\.mgf", "\\1", Spectrum.File)) %>% #parse the fraction
...
saveRDS(psms_data,'/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-jensen_46-individual-fractions-identification-data.rds') #save the data
```

Now that we have our saved identification object, we can start to look at the different identification patterns. First we can look at PSM identification rate across the fractions.

```{r}
psms = readRDS('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-jensen_46-individual-fractions-identification-data.rds') #grab the psms data
ms2_data = readRDS('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-jensen_all-fractions-ms2-data.rds') #grab the ms2 data
...
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-psms-rate-46fractions.pdf',psms_plot) #save the plot
message('Mean psms level across all fractions is ',mean(scans_table$psms,na.rm=TRUE),'.')
```

How does this translate to the actual identification rate (ie. number of PSMs / number of MS2 spectra)? 
...
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-identification-rate-46fractions.pdf',scans_plot) #save the plot
message('Mean identification rate across all fractions is ',mean(scans_table$id_rate,na.rm=TRUE),'.')
```

Hmm...so I wonder how well the MS is cycling? Is it triggering many MS2 scans in each cycle? Hitting the topN?
...
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-topN-rate-46fractions.pdf',topN_plot) #save the plot
message('Mean topN across all fractions is ',mean(ms_agg$topN,na.rm=TRUE),'.')
```

So definitely not hitting topN that frequently. What is the overall scan rate based on MS/MS per second?

```{r}
ms2_data = readRDS('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-jensen_all-fractions-ms2-data.rds') #grab the ms2 data
ms2_sub = subset(ms2_data, (ms2_data$RetentionTime >= 3) & (ms2_data$RetentionTime < 31)) #subset the data across a specific rt range
ms2_sub$seconds = ms2_sub$RetentionTime *60
ms2_sub$bin_group = cut(ms2_sub$seconds, breaks = seq(180,1860,1))
...
Hz_scatter = ggplot(ms2_agg_ordered, aes(bin_end,numMS2)) +
  ...
  scale_x_continuous(limits=c(0,2000),breaks=seq(0,2000,20))
Hz_scatter #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-Hz-46fractions.pdf',Hz_scatter) #save the plot
message('Mean number of MS/MS per second is ',mean(ms2_agg_ordered$numMS2,na.rm=TRUE),'.')
```

So this is not so unexpected when you combine all fractions. Might be better to look at individual duty cycles. What about the time between MS1 scans? Would it be ok for them to slow down with there 6.5 second base-to-base peak width?

```{r}
ms1_quant_files = list.files('/projects/ptx_results/OtherDataSets/2017-June_Jensen_CellSystems/hela_1mg/quant-output',pattern='*MS1ParseData.txt',full.names=TRUE)#grab the MS1 RawQuant fies
...
for (i in 1:length(ms1_quant_files)){
  ...
  ms1_in$fraction = sub(".*?Rep1_(.*?)_MS1ParseData.*", "\\1", ms1_quant_files[i]) #add a fraction column
...
saveRDS(ms1_data,'/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-jensen_all-fractions-ms1-data.rds') #save the data
...
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/parse-manuscript/Routput/bekker-rt-difference-46fractions.pdf',rt_diff_plot) #save the plot
...
And lastly, how does all of this translate to metrics based on injection times? Could we afford to slow down a bit?
