---
title: "Analysis of Orbitrap Fusion Lumos MS2 Data from Paulo *et al.*"
...
This file contains the code for processing the search and quantification output files from the triple knockout publication from Paulo *et al.* (PMID: 27400695). **These data were used to make portions of Figure S-2 - S-4.**
...
psm_files = list.files('/projects/ptx_results/OtherDataSets/2017-June_Paulo_TKO/reports-output',pattern='L.*ms2.*Default_PSM_Report.txt',full.names=TRUE)
#quant files from RawQuant algorithm
quant_files = list.files('/projects/ptx_results/OtherDataSets/2017-June_Paulo_TKO/quant-output',pattern='L.*ms2.*QuantData.txt',full.names=TRUE)
#quant files from Proteome Discoverer
pd_files = list.files('/projects/ptx_results/OtherDataSets/2017-June_Paulo_TKO/pd-quant-output',pattern='L.*ms2.*QuanSpectra.txt',full.names=TRUE)
```
...
for (i in 1:length(psm_files)){
  ...
  colnames(psm_f1) = c('accession_groups','sequence','ScanNumber','id_precursor_mz','id_charge','fraction','accession')
  ...
  quant_in[,9:68][quant_in[,9:68] == 0] = NA #transform any zero or missing values to NA
  ...
  quant_meta = quant_in[,c('PrecursorMass','ScanNumber','MS1ScanNumber','RetentionTime',
                           'PrecursorCharge','MS2IonInjectionTime','MS1Interference','tmt126_mass','tmt127N_mass',
  ...
  pd_in$fraction = paste(sub("(.*)\\.raw", "\\1", pd_in$fraction),'_ms2',sep='') #add a fraction column
...
saveRDS(psm_all, '/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo-lumos-ms2_all-psms-id-quant.rds')
```

Now we can plot some of the data to see how well the observations align with previously published results.

```{r}
psm_set = readRDS('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo-lumos-ms2_all-psms-id-quant.rds') #read in the processed data
...
met6pd_plot = ggplot(melt(met6[,c(53:61)]), aes(variable,value)) +
  ...
  geom_hline(yintercept=median(melt(met6[,c(53:55)])$value,na.rm=TRUE),color=brewer.pal(9,'YlOrRd')[8],linetype='dashed',size=1) +
  scale_y_continuous(limits=c(0,300),breaks=seq(0,300,50))
met6pd_plot #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo_met6_lumosM2-PD_boxplot.pdf',met6pd_plot) #save the plot


#make a boxplot for met6 using the RawQuant values
met6py_plot = ggplot(melt(met6[,c(26:34)]), aes(variable,value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha=0.5, size=0.8, colour='grey20') +
  labs(x = "TMT Channel", y = 'Signal-to-Noise', title = 'RawQuant') +
  theme(axis.text.x = element_text(hjust = 1, size = 9)) +
  geom_hline(yintercept=median(melt(met6[,c(26:28)])$value,na.rm=TRUE),color=brewer.pal(9,'YlOrRd')[8],linetype='dashed',size=1) +
  scale_y_continuous(limits=c(0,300),breaks=seq(0,300,50))
met6py_plot #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo_met6_lumosM2-PY_boxplot.pdf',met6py_plot) #save the plot

#make a boxplot for pfk2 using the Proteome Discoverer values
pfk2pd_plot = ggplot(melt(pfk2[,c(53:61)]), aes(variable,value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha=0.5, size=0.8, colour='grey20') +
  labs(x = "TMT Channel", y = 'Signal-to-Noise', title = 'Proteome Discoverer') +
  theme(axis.text.x = element_text(hjust = 1, size = 9)) +
  geom_hline(yintercept=median(melt(pfk2[,c(56:58)])$value,na.rm=TRUE),color=brewer.pal(9,'YlOrRd')[8],linetype='dashed',size=1) +
  scale_y_continuous(limits=c(0,350),breaks=seq(0,350,50))
pfk2pd_plot #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo_pfk2_lumosM2-PD_boxplot.pdf',pfk2pd_plot) #save the plot

#make a boxplot for pfk2 using the RawQuant values
pfk2py_plot = ggplot(melt(pfk2[,c(26:34)]), aes(variable,value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha=0.5, size=0.8, colour='grey20') +
  labs(x = "TMT Channel", y = 'Signal-to-Noise', title = 'RawQuant') +
  theme(axis.text.x = element_text(hjust = 1, size = 9)) +
  geom_hline(yintercept=median(melt(pfk2[,c(29:31)])$value,na.rm=TRUE),color=brewer.pal(9,'YlOrRd')[8],linetype='dashed',size=1) +
  scale_y_continuous(limits=c(0,350),breaks=seq(0,350,50))
pfk2py_plot #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo_pfk2_lumosM2-PY_boxplot.pdf',pfk2py_plot) #save the plot

#make a boxplot for ura2 using the Proteome Discoverer values
ura2pd_plot = ggplot(melt(ura2[,c(53:61)]), aes(variable,value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha=0.5, size=0.8, colour='grey20') +
  labs(x = "TMT Channel", y = 'Signal-to-Noise', title = 'Proteome Discoverer') +
  theme(axis.text.x = element_text(hjust = 1, size = 9)) +
  geom_hline(yintercept=median(melt(ura2[,c(59:61)])$value,na.rm=TRUE),color=brewer.pal(9,'YlOrRd')[8],linetype='dashed',size=1) + 
  scale_y_continuous(limits=c(0,160),breaks=seq(0,200,50))
ura2pd_plot #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo_ura2_lumosM2-PD_boxplot.pdf',ura2pd_plot) #save the plot

#make a boxplot for ura2 using the RawQuant values
ura2py_plot = ggplot(melt(ura2[,c(26:34)]), aes(variable,value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha=0.5, size=0.8, colour='grey20') +
  labs(x = "TMT Channel", y = 'Signal-to-Noise', title = 'RawQuant') +
  theme(axis.text.x = element_text(hjust = 1, size = 9)) +
  geom_hline(yintercept=median(melt(ura2[,c(32:34)])$value,na.rm=TRUE),color=brewer.pal(9,'YlOrRd')[8],linetype='dashed',size=1) +
  scale_y_continuous(limits=c(0,160),breaks=seq(0,200,50))
ura2py_plot #call the plot
save_plot('/projects/ptx_analysis/chughes/projects-current/raw-quant/published-data/paulo_TKO/Routput/paulo_ura2_lumosM2-PY_boxplot.pdf',ura2py_plot) #save the plot

#output the expression values for each of the plots
message(median(melt(met6[,c(53:55)])$value,na.rm=TRUE))
message(median(melt(met6[,c(26:28)])$value,na.rm=TRUE))
message(median(melt(pfk2[,c(56:58)])$value,na.rm=TRUE))
message(median(melt(pfk2[,c(29:31)])$value,na.rm=TRUE))
message(median(melt(ura2[,c(60:62)])$value,na.rm=TRUE))
message(median(melt(ura2[,c(32:34)])$value,na.rm=TRUE))
```
